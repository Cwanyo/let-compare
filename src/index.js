...
/**
 * Deep compare both values.
 *
 * @param {*} a
 * @param {*} b
 * @returns {boolean} Returns `true` if both values are same, else `false`.
 */
function letCompare(a, b) {
  try {
    return isEqual(a, b);
  } catch (error) {
    console.error("letCompare not working as expected!", error);
    return false;
  }
}

module.exports = letCompare;
